...
public class TweetsSearchServiceImpl implements TweetsSearchService {
	...
	public JSONObject searchRecentTweetsByKeywords(
			@NotNull(message = "Repository Names should not be NULL") final String gitHubProject) { 
		...
		try {
			...
			if (response != null) {
				...
				for (int index = 0; index < jsonArray.length(); index++) {
					...
					tweetMessages.append("screen_name",
